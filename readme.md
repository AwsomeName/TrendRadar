...
本项目提供了一个Web管理界面，方便用户通过浏览器查看和管理配置。
...
3. **访问管理界面**:
   - 打开浏览器访问: `http://localhost:38001`（或对应端口）
   - 界面包含以下功能模块：
     - **仪表盘**: 查看系统状态、配置文件状态、今日数据统计
     - **关键词管理**: 在线编辑 `frequency_words.txt` 配置文件
     - **配置管理**: 在线编辑 `config.yaml` 配置文件
     - **手动执行**: 手动触发一次数据抓取
     - **历史记录**: 查看历史执行记录和输出文件

#### Web界面功能说明

- **实时状态监控**: 显示配置文件状态、关键词文件状态、今日数据状态等
- **在线配置编辑**: 无需直接编辑文件，通过Web界面即可修改配置
- **一键执行**: 支持通过Web界面手动触发数据抓取
- **历史记录查看**: 方便查看之前的执行结果和输出文件
